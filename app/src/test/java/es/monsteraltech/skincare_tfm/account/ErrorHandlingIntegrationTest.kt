...
/**
 * Integration tests for error handling and loading states across account management components
 * Tests the interaction between AccountResult, RetryManager, and account managers
 */
class ErrorHandlingIntegrationTest {
    ...
    fun `AccountResult should work with retry manager for retryable errors`() = runTest {
        ...
        assertEquals(3, attemptCount) // Should retry twice then succeed
    }
    ...
    fun `AccountResult should work with retry manager for non-retryable errors`() = runTest {
        ...
        assertEquals(1, attemptCount) // Should not retry validation errors
    }
    ...
    fun `Error messages should be user-friendly in Spanish`() {
        ...
        // Ensure messages are not technical/developer-focused
