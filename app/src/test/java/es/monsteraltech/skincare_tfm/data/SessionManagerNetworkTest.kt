...
/**
 * Tests for SessionManager network error handling and retry logic
 * Focuses on offline verification and network resilience
 */
...
class SessionManagerNetworkTest {
    ...
    fun `should allow offline access with valid local token when network is unavailable`() = runTest {
        // Arrange - Create valid session data that hasn't expired
        ...
        // Mock Firebase to throw network error
    ...
    fun `should deny offline access with expired local token when network is unavailable`() = runTest {
        ...
        // Assert - Should deny access with expired token even offline
    ...
    fun `should retry token verification on network timeout`() = runTest {
        ...
        // Mock first calls to fail with timeout, then succeed
        ...
        // Verify that getIdToken was called multiple times (retries)
    ...
    fun `should retry token refresh on network errors`() = runTest {
        ...
        // Mock first calls to fail with network error, then succeed
    ...
    fun `should fail after maximum retry attempts on persistent network errors`() = runTest {
        ...
        // Mock all calls to fail with network error
        ...
        // Verify that getIdToken was called maximum number of times
        verify(mockFirebaseUser, times(3)).getIdToken(true) // MAX_RETRY_ATTEMPTS = 3
    }
    ...
    fun `should not retry on non-network errors`() = runTest {
        ...
        // Mock to fail with non-network error (e.g., authentication error)
        ...
        // Verify that getIdToken was called only once (no retries)
    ...
    fun `should handle mixed network and non-network errors correctly`() = runTest {
        ...
        // Mock first call to fail with network error, second with auth error
        ...
        // Assert - Should fail on auth error without further retries
        assertFalse(result, "Should fail on authentication error")
        
        // Verify that getIdToken was called twice (one retry for network, then auth error)
    ...
    fun `should maintain session during network errors if token refresh succeeds later`() = runTest {
        // Arrange
        val currentTime = System.currentTimeMillis()
        val soonToExpireTime = currentTime + TimeUnit.MINUTES.toMillis(3) // Will expire in 3 minutes
        ...
        // Mock refresh to succeed after network error
        ...
        // Assert - Should succeed after successful refresh
