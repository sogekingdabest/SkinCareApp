...
/**
 * Simple compilation test to verify SessionManager Firebase integration compiles correctly
 */
...
class SessionManagerCompilationTest {
    ...
    fun `SessionManager methods should be callable without exceptions`() = runTest {
        // Test that all methods can be called without compilation errors
        try {
            ...
            // If we reach here, all methods compiled and executed without throwing
            assertNotNull(sessionManager, "All SessionManager methods should be callable")
        } catch (e: Exception) {
            // Methods may fail at runtime due to missing Firebase setup, but should compile
