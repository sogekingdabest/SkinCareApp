...
/**
 * Tests de rendimiento para SessionManager
 * Verifica que los tiempos de inicio y operaciones sean aceptables
 */
...
class SessionPerformanceTest {
    ...
    companion object {
        // Tiempos máximos aceptables en milisegundos
    ...
    fun testSessionVerificationPerformance() = runBlocking {
        // Medir tiempo de verificación de sesión sin datos
    ...
    fun testFastModePerformance() = runBlocking {
        // Medir tiempo de verificación en modo rápido
    ...
    fun testCacheAccessPerformance() = runBlocking {
        // Primero obtener datos para poblar cache
        ...
        val cacheAccessTime = measureTimeMillis {
            ...
            // El resultado puede ser null, eso está bien para esta prueba
        }
    ...
    fun testSessionClearPerformance() = runBlocking {
        // Medir tiempo de limpieza de sesión
    ...
    fun testCacheEffectiveness() = runBlocking {
        ...
        // Medir tiempo con cache (segunda llamada)
    ...
    fun testCacheStatsPerformance() {
        // Medir tiempo de obtención de estadísticas de cache
        ...
        assertTrue(
            ...
            statsTime < 50L // Debería ser muy rápido
        )
    ...
    fun testCacheClearPerformance() {
        // Poblar cache primero
        runBlocking {
            sessionManager.getStoredSession()
        }

        // Medir tiempo de limpieza de cache
        val clearTime = measureTimeMillis {
            sessionManager.clearCache()
        }
        ...
        assertTrue(
            ...
            clearTime < 50L // Debería ser muy rápido
        )
    ...
    fun testStartupTimeSimulation() = runBlocking {
        // Simular el flujo completo de inicio de aplicación
        val startupTime = measureTimeMillis {
            // 1. Verificar sesión (como lo haría SessionCheckActivity)
            val isValid = sessionManager.isSessionValid(fastMode = true)
            
            // 2. Obtener estadísticas de cache
            val stats = sessionManager.getCacheStats()
            
            // 3. Si no hay sesión válida, limpiar
            if (!isValid) {
                sessionManager.clearSession()
            }
        }
    ...
    fun testMemoryEfficiency() {
        // Verificar que las estadísticas de cache no crezcan indefinidamente
        ...
        // Las estadísticas deberían mantenerse en un tamaño razonable
        assertTrue(
            "Estadísticas de cache crecieron demasiado",
            finalStats.size <= initialStats.size + 2 // Permitir algunos campos adicionales
        )
